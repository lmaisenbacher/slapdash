...
export default class NumericInput extends React.Component
{
    static propTypes = {
        ...
        onNumericKeyDown : PropTypes.func,
    ...
    _propsToState(props: Object) {
        ...
        if (props.hasOwnProperty("value")) {
            ...
            if (Object.is(props.value, -0)) { // preserve -0
                out.stringValue = '-' + out.stringValue;
            }

            out.value = out.stringValue !== '' ?
                // this._parse(props.value) :
                this._parse(out.stringValue) :
                null
        }
     ...
     * string).
     * 
     * We now pass the signed `step` to a precision function to signify that this is
     * being called as part of a new-value calculation.
     */
    _toNumber(x: any, step: number=0): number
    {
        let n = parseFloat(x);
        ...
        if (this._isStrict) {
            let precision = access(this.props, "precision", null, this, step);
     ...
     * @private
     */
    _format(n: number, s: number): string
    {
        // s (step) is the absolute value of the last step
        // console.log('=== _format is given number ' + n + ' and lastStep ' + s);
        let _n = this._toNumber(n) + '';
        let precision = access(this.props, "precision", null, this, 0);
        if (precision !== null) {
            _n = n.toFixed(precision);
            if (Object.is(n, -0)) { // preserve -0
                _n = '-' + _n;
            }
        }
        // console.log('  from precision, number formatted to ' + _n);

        if (this.props.format) {
            // console.log('  from formatter, number formatted to ' + this.props.format(_n, s));
            return this.props.format(_n, s);
        }
    ...
    _step(n: number, callback?: Function): boolean
    {
        ...
        let _n = this._toNumber((this.state.value || 0) + _step * n, Math.sign(n)*_step);
        ...
        if (_n !== this.state.value) {
            this.setState({ value: _n, stringValue: _n + "", lastStep: _step }, callback);
            return true;
        }
    ...
    _onKeyDown(...args: Array<any>): void
    {
        ...
        if (!e.isDefaultPrevented()) {
            if (e.key === "ArrowUp") {
                ...
                //     // if we are removing a decimal from the left, move cursor left to account for the lost space
                ...
                //     // if we are removing a decimal from the right, move cursor right to account for the lost space
                //     if (this.refsInput.selectionStart == this.refsInput.selectionEnd &&
                ...
                //     }
                // }
                if (!isNaN(e.key)) { // typing a number
                    access(this.props, "onNumericKeyDown", null, e, this);
                }
            }
        }
    }
    ...
    render()
    {
        ...
        let {
            ...
            value, type, style, defaultValue, onInvalid, onValid, strict, noStyle, onNumericKeyDown,
        ...
        if (loose && RE_INCOMPLETE_NUMBER.test(stringValue)) {
            ...
            // console.log('[RENDER] Component not re-rendering because incomplete number');
        }

        // Not a number and not empty (loose mode only)
        else if (loose && stringValue && !RE_NUMBER.test(stringValue)) {
            attrs.input.value = stringValue;
            // console.log('[RENDER] Component not re-rendering because not a number');
        }

        // number
        else if (state.value || state.value === 0) {
            attrs.input.value = this._format(state.value, state.lastStep);
            // console.log('[RENDER] Component re-rendering because formatting number');
        }
